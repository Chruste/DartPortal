...
try {
    ...
    portal_log_error('Cloud Storage DB Fehler', $exception);
    portal_json_response(['success' => false, 'message' => 'Datenbank nicht erreichbar.'], 500);
}

/**
 * Spiel-Registrierung und Konfiguration
 */
function portal_cloud_storage_config(string $gameType): array
{
    ...
    $map = [
        'shanghai21' => [
            ...
            'defaultSaveName' => 'Shanghai 21',
            'participantFields' => ['currentTargetIndex', 'currentTotalScore'],
        ],
        'shanghai42' => [
            'sessions' => 'shanghai42_sessions',
            'participants' => 'shanghai42_session_participants',
            'events' => 'shanghai42_events',
            'defaultSaveName' => 'Shanghai 42',
            'participantFields' => ['currentTargetIndex', 'currentTotalScore'],
        ],
        'darts501' => [
            'sessions' => 'darts501_sessions',
            'participants' => 'darts501_session_participants',
...
/**
 * Generische Normalisierungsfunktionen
 */
function portal_cloud_default_save_name(string $gameType): string
{
    $config = portal_cloud_storage_config($gameType);
    return $config['defaultSaveName'];
}

function portal_cloud_normalize_save_name(mixed $value, string $gameType): string
{
    $saveName = trim((string) ($value ?? ''));
    if ($saveName === '') {
        $saveName = portal_cloud_default_save_name($gameType);
    }
...
function portal_cloud_normalize_portal_user_id(mixed $value): ?int
{
    if (!is_numeric($value)) {
        return null;
    }
...
function portal_cloud_normalize_participants(array $state, int $ownerUserId, array $config): array
{
    ...
    foreach ($players as $fallbackSeatNo => $player) {
        ...
        $portalUserId = portal_cloud_normalize_portal_user_id(
        ...
        $invitedByUserId = portal_cloud_normalize_portal_user_id($player['invitedByUserId'] ?? null);
        ...
        foreach ($participantFields as $field) {
            if ($field === 'currentTargetIndex') {
                $participantData[$field] = isset($player['currentIndex']) && is_numeric($player['currentIndex'])
                    ? max(0, (int) $player['currentIndex'])
                    : 0;
            } elseif ($field === 'currentTotalScore') {
                $participantData[$field] = isset($player['totalScore']) && is_numeric($player['totalScore'])
                    ? (int) $player['totalScore']
                    : 0;
            } elseif ($field === 'currentRemainingScore') {
                $participantData[$field] = isset($player['remainingScore']) && is_numeric($player['remainingScore'])
                    ? max(0, (int) $player['remainingScore'])
                    : 501;
            }
        }

        $participants[$seatNo] = $participantData;
    }
...
function portal_cloud_build_participant_summary(array $participants): string
{
    $totalsByName = [];
    ...
    foreach ($participants as $participant) {
        ...
        // Versuche verschiedene Scoring-Felder
        $score = 0;
        if (isset($participant['currentTotalScore']) && is_numeric($participant['currentTotalScore'])) {
            $score = (int) $participant['currentTotalScore'];
        } elseif (isset($participant['totalScore']) && is_numeric($participant['totalScore'])) {
            $score = (int) $participant['totalScore'];
        } elseif (isset($participant['currentRemainingScore']) && is_numeric($participant['currentRemainingScore'])) {
            // Für Darts501: höhere Punktzahl am Ende ist besser, also 501-remaining
            $remaining = (int) $participant['currentRemainingScore'];
            $score = max(0, 501 - $remaining);
        }

        $totalsByName[$name] = ($totalsByName[$name] ?? 0) + $score;
    }
...
function portal_cloud_resolve_participant_summary(array $row): string
{
    $participantsJson = trim((string) ($row['participants_json'] ?? ''));
    if ($participantsJson !== '') {
        $participants = json_decode($participantsJson, true);
        if (is_array($participants)) {
            $summary = portal_cloud_build_participant_summary($participants);
            if ($summary !== '') {
                return $summary;
            }
        }
...
function portal_cloud_state_meta(array $state, int $ownerUserId, array $config): array
{
    $seatParticipants = portal_cloud_normalize_participants($state, $ownerUserId, $config);
    $participants = [];
    ...
    return [
        'participantSummary' => portal_cloud_build_participant_summary($seatParticipants),
...
function portal_cloud_state_json(array $state): string
{
...
function portal_cloud_sync_participants(mysqli $db, string $table, int $sessionId, array $participants, array $config): array
{
    ...
    // Dynamische INSERT mit game-spezifischen Feldern
    $fields = ['session_id', 'seat_no', 'portal_user_id', 'invited_by_user_id', 'display_name', 'participant_role', 'invitation_status'];
    $values = ['?', '?', '?', '?', '?', '?', '?'];
    $bindTypes = 'iiiisss';
    ...
    $insertStmt = $db->prepare(
        "INSERT INTO {$table} ({$fieldStr}) VALUES ({$valueStr})"
    );
    if (!$insertStmt) {
    ...
    $updateFields[] = 'last_throw_at = COALESCE(?, last_throw_at)';
    $updateBindTypes .= 's';

    $updateFieldStr = implode(',', $updateFields);
    ...
    foreach ($participants as $participant) {
        ...
        // Sammle game-spezifische Felder
        $gameSpecificValues = [];
        $hasActivity = false;
        foreach ($config['participantFields'] ?? [] as $field) {
            $value = isset($participant[$field]) ? (int) $participant[$field] : 0;
            $gameSpecificValues[] = $value;
            if ($value > 0) {
                $hasActivity = true;
            }
        }
        $lastThrowAt = $hasActivity ? $now : null;
        ...
        if (isset($existingBySeat[$seatNo])) {
            ...
            array_push($bindArr, ...$gameSpecificValues);
            $bindArr[] = $lastThrowAt;
            $bindArr[] = $participantId;
...
function portal_cloud_normalize_event(array $event, array $participantLookup = [], int $recordedByUserId = 0): array
{
    ...
    $playerUserId = portal_cloud_normalize_portal_user_id(
...
function portal_cloud_fetch_session_for_user(mysqli $db, array $config, int $saveId, int $userId, bool $forUpdate = false): ?array
{
    $sql = "SELECT
                s.id,
                s.owner_user_id,
...
function portal_cloud_fetch_pending_invitation(mysqli $db, array $config, int $saveId, int $userId, bool $forUpdate = false): ?array
{
    $fieldList = 's.id, s.owner_user_id, s.save_name, s.participant_summary, s.participants_json, s.updated_at, s.state_json, s.event_count, s.last_event_at, s.participant_count, s.is_archived, p.id AS participant_id, p.seat_no, p.display_name, p.invited_by_user_id, p.participant_role, p.invitation_status';
...
function portal_cloud_fetch_participant_by_user(mysqli $db, string $table, int $sessionId, int $portalUserId, array $config, bool $forUpdate = false): ?array
{
    $fieldList = 'id, seat_no, portal_user_id, invited_by_user_id, display_name, participant_role, invitation_status, is_active';
...
function portal_cloud_fetch_next_seat_no(mysqli $db, string $table, int $sessionId, bool $forUpdate = false): int
{
...
function portal_cloud_state_has_player(array $state, int $portalUserId): bool
{
    $players = $state['players'] ?? [];
    if (!is_array($players)) {
        return false;
...
function portal_cloud_append_accepted_player(array $state, array $participant, array $config): array
{
    $players = $state['players'] ?? [];
    if (!is_array($players)) {
        $players = [];
    ...
    foreach ($config['participantFields'] ?? [] as $field) {
        if ($field === 'currentTargetIndex') {
            $playerData[$field] = isset($participant['current_target_index']) ? (int) $participant['current_target_index'] : 0;
        } elseif ($field === 'currentTotalScore') {
            $playerData[$field] = isset($participant['current_total_score']) ? (int) $participant['current_total_score'] : 0;
        } elseif ($field === 'currentRemainingScore') {
            $playerData[$field] = isset($participant['current_remaining_score']) ? (int) $participant['current_remaining_score'] : 501;
        }
    }
...
function portal_cloud_resolve_invite_candidate(mysqli $db, array $config, int $sessionId, array $friend): array
{
    $friendId = (int) ($friend['id'] ?? 0);
    $participant = $friendId > 0
        ? portal_cloud_fetch_participant_by_user($db, $config['participants'], $sessionId, $friendId, $config)
        : null;
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        $config = portal_cloud_storage_config((string) ($_GET['gameType'] ?? ''));
        ...
        if ($action === 'list') {
            ...
            while ($result && ($row = $result->fetch_assoc())) {
                $saves[] = [
                    'id' => (int) $row['id'],
                    'saveName' => (string) ($row['save_name'] ?? portal_cloud_default_save_name($config['gameType'])),
                    'updatedAt' => portal_format_datetime((string) ($row['updated_at'] ?? '')),
                    'participantSummary' => portal_cloud_resolve_participant_summary($row),
        ...
        if ($action === 'load') {
            ...
            $row = portal_cloud_fetch_session_for_user($mysqli_user, $config, $saveId, $userId);
            if ($row === null) {
        ...
        if ($action === 'invite_candidates') {
            ...
            $sessionRow = portal_cloud_fetch_session_for_user($mysqli_user, $config, $saveId, $userId);
            if ($sessionRow === null) {
                portal_json_response(['success' => false, 'message' => 'Spielstand nicht gefunden.'], 404);
            }
            ...
            foreach ($friends as $friend) {
                $candidates[] = portal_cloud_resolve_invite_candidate($mysqli_user, $config, $saveId, $friend);
            }
            ...
            portal_json_response([
                ...
                'save' => [
                    'id' => $saveId,
                    'saveName' => (string) ($sessionRow['save_name'] ?? portal_cloud_default_save_name($config['gameType'])),
    ...
    $config = portal_cloud_storage_config((string) ($input['gameType'] ?? ''));
    $action = strtolower(trim((string) ($input['action'] ?? '')));

    if ($action === 'create') {
        $state = isset($input['state']) && is_array($input['state']) ? $input['state'] : [];
        $stateJson = portal_cloud_state_json($state);
        $meta = portal_cloud_state_meta($state, $userId, $config);
        $saveName = portal_cloud_normalize_save_name($input['saveName'] ?? '', $config['gameType']);
        ...
        try {
            ...
            portal_cloud_sync_participants($mysqli_user, $config['participants'], $saveId, $meta['participants'], $config);
    ...
    if ($action === 'copy') {
        ...
        $sessionRow = portal_cloud_fetch_session_for_user($mysqli_user, $config, $sourceSaveId, $userId);
        if ($sessionRow === null) {
            portal_json_response(['success' => false, 'message' => 'Spielstand nicht gefunden.'], 404);
        }
        ...
        $saveName = portal_cloud_normalize_save_name(
            $input['saveName'] ?? ((string) ($sessionRow['save_name'] ?? portal_cloud_default_save_name($config['gameType'])) . ' (Kopie)'),
            $config['gameType']
        );
        $stateJson = portal_cloud_state_json($copiedState);
        $meta = portal_cloud_state_meta($copiedState, $userId, $config);
    ...
    if ($action === 'update') {
        ...
        try {
            $sessionRow = portal_cloud_fetch_session_for_user($mysqli_user, $config, $saveId, $userId, true);
            if ($sessionRow === null) {
            ...
            $saveName = array_key_exists('saveName', $input)
                ? portal_cloud_normalize_save_name($input['saveName'], $config['gameType'])
                : portal_cloud_normalize_save_name((string) ($sessionRow['save_name'] ?? ''), $config['gameType']);
            $meta = portal_cloud_state_meta($state, $ownerUserId, $config);
            $participantLookup = portal_cloud_sync_participants($mysqli_user, $config['participants'], $saveId, $meta['participants'], $config);
    ...
    if ($action === 'rename') {
        ...
        portal_json_response([
            ...
            'save' => [
                ...
                'participantSummary' => portal_cloud_resolve_participant_summary($sessionRow),
                'isArchived' => !empty($sessionRow['is_archived']),
                'isOwner' => true,
            ],
        ]);
    ...
    if ($action === 'invite_friend') {
        ...
        try {
            ...
            $participantRow = portal_cloud_fetch_participant_by_user($mysqli_user, $config['participants'], $saveId, $targetUserId, $config, true);
            ...
            if ($participantRow !== null) {
                ...
                // Dynamisches UPDATE mit game-spezifischen Feldern
                $updateFields = [
                    'invited_by_user_id = ?',
                    'display_name = ?',
                ...
                $updateBindTypes = 'is';
                $updateBindArr = [$userId, (string) ($targetUser['display_name'] ?? 'Spieler')];
                ...
                $updateFields[] = 'joined_at = NULL';
                $updateFields[] = 'last_throw_at = NULL';
                ...
                $updateBindArr[] = (int) $participantRow['id'];
                $updateStmt->bind_param($updateBindTypes . 'i', ...$updateBindArr);
                $updateStmt->execute();
                $updateStmt->close();
            } else {
                $seatNo = portal_cloud_fetch_next_seat_no($mysqli_user, $config['participants'], $saveId, true);
                
                $fields = ['session_id', 'seat_no', 'portal_user_id', 'invited_by_user_id', 'display_name', "participant_role"];
                $values = ['?', '?', '?', '?', '?', "'friend'"];
                $bindTypes = 'iiiis';
                $bindArr = [$saveId, $seatNo, $targetUserId, $userId, (string) ($targetUser['display_name'] ?? 'Spieler')];
                ...
                $insertStmt = $mysqli_user->prepare(
                    "INSERT INTO {$config['participants']} ({$fieldStr}) VALUES ({$valueStr})"
                );
                if (!$insertStmt) {
                    throw new RuntimeException('Einladung konnte nicht gespeichert werden.');
                }

                $insertStmt->bind_param($bindTypes, ...$bindArr);
                $insertStmt->execute();
                $insertStmt->close();
            }
    ...
    if ($action === 'accept_invitation') {
        ...
        try {
            $invitationRow = portal_cloud_fetch_pending_invitation($mysqli_user, $config, $saveId, $userId, true);
            ...
            $participantRow = portal_cloud_fetch_participant_by_user($mysqli_user, $config['participants'], $saveId, $userId, $config, true);
            if ($participantRow === null) {
                throw new RuntimeException('Teilnehmer konnte nach Annahme nicht geladen werden.');
            }

            if (!portal_cloud_state_has_player($state, $userId)) {
                $state = portal_cloud_append_accepted_player($state, $participantRow, $config);
            }
